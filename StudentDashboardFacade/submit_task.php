...
$facade = new StudentDashboardFacade($conn, $username);
$message = '';
$messageType = '';
$task = null;
$errors = [];
...
if (isset($_GET['id'])) {
    try {
        $taskID = $_GET['id'];
        $taskObj = new Task($conn); // نحتاج إلى كائن Task لجلب تفاصيل المهمة
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit'])) {
    try {
        ...
        if (empty($errors)) {
            // استخدام الواجهة لتسليم المهمة
            $result = $facade->submitTaskWithFile($taskID, $username, $notes, $_FILES['task_file']);
